...
const getcourses = async (req, res) => {
    try {
        const allcource = await course.find().select("-videos -quizs")
        if (!allcource) {
...
const getmycourse = async (req, res) => {
    ...
    try {
        ...
        const mycoures =  await user.findById(userId).populate('courses','-prise -instructor -students -Syllabus -discount -originalprise -updatedAt -createdAt -__v').select('courses')   
...
const getcoursesStudent = async (req, res) => {
    ...
    try {
        ...
        const courseStudent = await course.find({ _id: courseId }).select('students').populate('students','_id mobile profilePic email name isBan')
