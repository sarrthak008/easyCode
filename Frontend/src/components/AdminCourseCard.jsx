...
const AdminCourseCard = ({ course }) => {
  ...
  const gotoApprovedAssignment = () => {
    navigate(`/approveAssignment/${course._id}`)
  }
  
  return (
    <div key={course._id} className='bg-gray-800 w-[340px] h-[500px] rounded-lg  p-1 text-white relative mb-5'>
      ...
      <div className='mt-6 flex flex-col gap-4 m-auto'>
        ...
        <button className='bg-[#00DB80] text-white w-[81%] ml-4 py-2 text-2xl text-black rounded-sm hover:bg-green-700 shadow-lg shadow-gray-900 ' onClick={()=>gotosetting()}>Main Settings</button>

        <button className='bg-[#00DB80] text-white w-[81%] ml-4 py-2 text-2xl text-black rounded-sm hover:bg-green-700 shadow-lg shadow-gray-900 ' onClick={()=>gotoApprovedAssignment()}>Assignment Settings</button>
      </div>
